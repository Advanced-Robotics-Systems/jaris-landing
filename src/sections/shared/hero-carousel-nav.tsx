    ...
    <div className="relative w-full h-[42vh] lg:h-[calc(100vh-70px)]  pb-10 sm:pb-20 lg:pb-36 bg-jaris-blue pt-10">
      ...
      <div className="absolute w-full h-[30%] md:h-[23%] bottom-4 md:bottom-10 lg:bottom-16 xl:bottom-20 flex items-center justify-around z-30">
        {carouselNavItems.map((item: any, index: number) => (
          <Card
            key={index}
            className={`basis-[22.5%] ${
              active === index ? "bg-jaris-gold" : "bg-jaris-gold-dark"
            } h-full text-jaris-blue md:p-4 lg:p-6 hover:bg-jaris-gold-dark/70 rounded-md`}
            isPressable
            onPress={() => setActive(index)}
          >
            <CardBody className="items-center justify-center overflow-hidden px-1">
              <p className="small-text text-white text-center">
                {t(`hero.newsNav.${item.subtitle}`)}
              </p>
              <p className="w-full text-center subtitle xl:heading-secondary font-semibold text-jaris-white">
                {t(`hero.newsNav.${item.title}`)}
              </p>
            </CardBody>
          </Card>
        ))}
